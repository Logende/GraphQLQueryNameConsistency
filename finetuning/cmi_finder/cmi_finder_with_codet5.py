...
class FineTuneModel():
    ...
    def prepare_features(self, examples, input_label="source", target_label="target", max_length=512):
        ...
        model_inputs = self.tokenizer(inputs, max_length=self.tokenizer.model_max_length, padding="max_length",
                                      truncation=True)
        labels = self.tokenizer(targets, max_length=self.tokenizer.model_max_length, padding="max_length",
                                truncation=True)
